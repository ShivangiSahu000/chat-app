...
io.on('connection', (socket) => {
    ...
    // Handle the 'setUsername' event from the client to set the username
    socket.on('setUsername', (name) => {
        username = name;
        io.emit('message', `${username} has joined the chat`);  // Broadcast to all clients that a new user has joined
    });

    // Handle the 'message' event from the client to broadcast messages
    socket.on('message', (message) => {
        io.emit('message', `${username}: ${message}`);  // Broadcast the message to all clients
    });

    // Handle client disconnections
    socket.on('disconnect', () => {
        console.log('Client disconnected'); // Log when a client disconnects
        io.emit('message', `${username} has left the chat`);    // Broadcast to all clients that a user has left
    });
});
